 ...
 *
 * Assembles and drives the HUD for the sentry.
 *
 * Same structure as `InfantryDrawCommand`, but the sentry has no operator of its own, so the
 * display is there for whoever is watching its feed; see that class for how the graphics tree and
 * the update cycle fit together.
 */
