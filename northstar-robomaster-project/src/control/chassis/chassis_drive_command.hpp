 ...
 * @ingroup chassis
 *
 * Drives the chassis from operator input, with translation interpreted relative to where the
 * turret is pointing.
 *
 * This is the default drive mode: pushing the stick forward moves the robot in the direction the
 * operator is looking down the turret, no matter which way the chassis happens to be facing.
 * Runs until interrupted, and stops the chassis when it ends.
 */
...
public:
    ...
    /// rotation, to the chassis in turret-relative terms.
