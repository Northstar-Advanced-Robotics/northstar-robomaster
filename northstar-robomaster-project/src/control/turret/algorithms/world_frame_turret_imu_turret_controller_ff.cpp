...
#include "world_frame_turret_imu_turret_controller_ff.hpp"
...
/**
 * Computes a feedforward term meant to overcome static friction (stiction) when the turret is
 * being commanded away from its current position but hasn't started moving yet. Only kicks in
 * when the turret is nearly stationary and the position error is outside a small deadzone, so it
 * doesn't fight the controller once the turret is already moving or dither while holding setpoint.
 *
 * @param[in] positionControllerError Current position error, in radians.
 * @param[in] velocityMeasured Current measured angular velocity, in radians/second.
 * @param[in] staticFrictionFFGain Feedforward output applied to overcome static friction.
 * @param[in] velocityDeadzone Below this measured angular velocity (radians/second) the turret is
 * considered stationary.
 * @param[in] positionErrorDeadzone Below this position error (radians) the turret is considered
 * at setpoint and no feedforward is applied.
 * @return The signed static friction feedforward term to add to the controller output.
 */
static inline float computeStaticFrictionFF(
    const float positionControllerError,
    const float velocityMeasured,
    const float staticFrictionFFGain,
    const float velocityDeadzone,
    const float positionErrorDeadzone)
{
    if (fabsf(velocityMeasured) > velocityDeadzone ||
        fabsf(positionControllerError) < positionErrorDeadzone)
    {
        return 0.0f;
    }

    return tap::algorithms::getSign(positionControllerError) * staticFrictionFFGain;
}
 ...
 * @param[out] velocityPid Velocity PID controller.
 * @param[in] staticFrictionFFGain Feedforward output applied to overcome static friction.
 * @param[in] staticFrictionVelocityDeadzone Below this measured angular velocity (radians/second)
 * the turret is considered stationary and eligible for the static friction feedforward term.
 * @param[in] staticFrictionErrorDeadzone Below this position error (radians) the turret is
 * considered at setpoint and no static friction feedforward is applied.
 * @return desired PID output from running the position -> velocity cascade controller
 */
float staticFrictionFF = 0;
...
void WorldFrameYawTurretImuCascadePidTurretControllerFF::initialize()
{
...
void WorldFrameYawTurretImuCascadePidTurretControllerFF::runController(
    const uint32_t dt,
    const WrappedFloat desiredSetpoint)
{
    ...
    const float pidOut = runWorldFrameTurretImuController(
        ...
        positionPid,
        velocityPid,
        world_rel_turret_imu::YAW_STATIC_FRICTION_FF_GAIN,
...
WrappedFloat WorldFrameYawTurretImuCascadePidTurretControllerFF::getMeasurement() const
{
    return Angle(worldFrameMeasurementIMU + M_TWOPI * IMUrevolutions);
}

WrappedFloat WorldFrameYawTurretImuCascadePidTurretControllerFF::getMeasurementMotor() const
{
...
void WorldFramePitchTurretImuCascadePidTurretControllerFF::initialize()
{
    initializeWorldFrameTurretImuController(
        this,
        getBmi088Pitch(),
...
void WorldFramePitchTurretImuCascadePidTurretControllerFF::runController(  // TODO for actual use
    ...
    const WrappedFloat desiredSetpoint)
{
    ...
    float pidOut = runWorldFrameTurretImuController(
        ...
        positionPid,
        velocityPid,
        world_rel_turret_imu::PITCH_STATIC_FRICTION_FF_GAIN,
        world_rel_turret_imu::STATIC_FRICTION_FF_VELOCITY_DEADZONE,
        world_rel_turret_imu::STATIC_FRICTION_FF_ERROR_DEADZONE);
